...
resource "proxmox_lxc" "singlesc" {
  ...
  network {
    ...
    ip     = "${cidrhost(var.pm_ct_network_subnet, 20)}/${var.pm_network_mask}"
  ...
  provisioner "local-exec" {
    command = "ANSIBLE_CONFIG=../ansible/ansible.cfg ansible-playbook -e 'ansible_ssh_private_key_file=../kredentzialak/gakoa' -i ${cidrhost(var.pm_ct_network_subnet, 20)}, ../ansible/docker-install.yaml -vvv"
  }
}
// ——————————————————————————————————————————————————————————
// Hacer APT update
...
resource "null_resource" "kaffka" {
  depends_on = [
    proxmox_lxc.singlesc
  ]

  provisioner "local-exec" {
    command = "ANSIBLE_CONFIG=../ansible/ansible.cfg ansible-playbook -e 'ansible_ssh_private_key_file=../kredentzialak/gakoa' -i ${cidrhost(var.pm_ct_network_subnet, 20)}, ../ansible/kaffka-docker.yaml -vvv"
  }
}

resource "null_resource" "security" {
  depends_on = [
null_resource.kaffka
  ]
