...
resource "proxmox_lxc" "singlesc" {
  ...
  network {
    ...
    ip     = "${var.pm_ct_ip}/${var.pm_network_mask}"
  ...
  provisioner "local-exec" {
    command = "ANSIBLE_CONFIG=../ansible/ansible.cfg ansible-playbook -e 'ansible_ssh_private_key_file=../kredentzialak/gakoa' -i ${cidrhost(var.pm_ct_network_subnet, 181)}, ../ansible/docker-install.yaml -vvv"
  }
}
// ——————————————————————————————————————————————————————————
// Hacer APT update
